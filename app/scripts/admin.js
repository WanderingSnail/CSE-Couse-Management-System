...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document
    .getElementById("classFormSection")
    .addEventListener("submit", async (e) => {
      e.preventDefault();
      const data = new FormData(e.target);
      ...
      <td><button class="btn-cancel" onclick="cancelClass('${
        newClass.classId
      }')">Cancel</button></td>
    `;
...
window.cancelClass = async function (classId, courseId) {
  const classes = await Class.load();
